...
def augment_images(image_path, output_dir, num_samples=10):
    ...
    for i in range(num_samples):
        aug_img = img
        for t in np.random.choice(transforms, size=2):  # Random 2 transforms per variation
            aug_img = t(aug_img)
...
def extract_and_save(reg_no, dept, year, section, model):
    ...
    embeddings = []
    # Augment all photos and collect all variations
    ...
    for img_file in os.listdir(aug_dir):
        ...
        for (x, y, w, h) in faces:
            ...
            face_tensor = torch.tensor(face_resized.astype(np.float32) / 255.0).unsqueeze(0).unsqueeze(0)
            with torch.no_grad():
    ...
    print(f"Saved embedding for {reg_no} with {len(embeddings)} images")
